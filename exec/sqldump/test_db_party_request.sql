...
CREATE TABLE `party_request` (
  `party_request_id` bigint NOT NULL AUTO_INCREMENT,
  `request_date` bigint NOT NULL,
  `request_status` enum('Approved','Refused','Waiting') DEFAULT NULL,
  `status_changed_date` bigint NOT NULL,
  `user_team_user_team_id` bigint DEFAULT NULL,
  PRIMARY KEY (`party_request_id`),
  KEY `FKbopy216yp8blgu2d6r04lrcnm` (`user_team_user_team_id`),
  CONSTRAINT `FKbopy216yp8blgu2d6r04lrcnm` FOREIGN KEY (`user_team_user_team_id`) REFERENCES `user_team` (`user_team_id`)
...
LOCK TABLES `party_request` WRITE;
/*!40000 ALTER TABLE `party_request` DISABLE KEYS */;
/*!40000 ALTER TABLE `party_request` ENABLE KEYS */;
UNLOCK TABLES;
